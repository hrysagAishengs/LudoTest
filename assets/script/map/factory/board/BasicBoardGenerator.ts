...
import { IBoardGenerator } from '../def/FactoryDef';
...
export class BasicBoardGenerator implements IBoardGenerator{
    ...
    // === 棋盤配置常量 ===
    private readonly GRID_SIZE = 15; // 棋盤為 15x15 的格子
    private readonly DESIGN_HEIGHT = 720; // 設計解析度的高度（像素）
    private readonly CELL_SIZE = 50; // 單個格子的視覺大小（像素）
    
    /**
     * 格子間距（中心點到中心點的距離）
     * 計算公式：(設計高度 - 格子大小) / (格子數 - 1)
     * 
     * 解釋：
     * - 15個格子需要14個間距
     * - 可用空間 = 720 - 50 = 670 像素
     ...
     */
    private readonly CELL_SPACING = (this.DESIGN_HEIGHT - this.CELL_SIZE) / (this.GRID_SIZE - 1);
    ...
    private createPrefabGrid():void {
        // 計算偏移量，讓 15x15 的中心點剛好在 Node 的 (0,0) 位置
        const offset = (this.GRID_SIZE - 1) * this.CELL_SPACING / 2;
     ...
     * 2. CELL_SPACING ≈ 47.86 像素（由 (720-50)/14 計算得出）
     * 3. 通過偏移量讓整個棋盤居中在 (0, 0) 位置
     */
    private createPosGridOnly(): void {
        // 計算偏移量，讓 15x15 的中心點剛好在 (0,0) 位置
        const offset = (this.GRID_SIZE - 1) * this.CELL_SPACING / 2;
        ...
        console.log(`棋盤生成配置：
            - 格子數量: ${this.GRID_SIZE} x ${this.GRID_SIZE}
            - 格子大小: ${this.CELL_SIZE} px
